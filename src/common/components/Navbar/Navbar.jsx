    ...
    return (
        <Box
            ...
            sx={{
                ...
                p: '10px 12px', textDecoration: 'none', borderRadius: '16px',
                transition: 'all 0.2s ease',
                '&:hover': {
                    bgcolor: 'primary.main',
                    boxShadow: '0 8px 24px rgba(0,0,0,0.1)',
                    transform: 'translateY(-2px)',
                    '& .sidebar-icon-box': { bgcolor: 'secondary.main', color: 'primary.main', transform: 'rotate(5deg) scale(1.1)', boxShadow: '0 4px 15px rgba(0,0,0,0.15)' },
                    '& .sidebar-label': { color: 'white' },
                },
                '&:active': { transform: 'scale(0.98)' },
            }}
        >
            <Box className="sidebar-icon-box" sx={{ width: 40, height: 40, borderRadius: '12px', bgcolor: 'primary.main', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'secondary.main', flexShrink: 0, transition: 'all 0.3s ease' }}>
                <ItemIcon size={20} />
            </Box>
            <Typography className="sidebar-label" sx={{ fontSize: '14px', fontWeight: 700, color: 'text.primary', transition: 'color 0.2s ease' }}>
                {item.label}
            </Typography>
        </Box>
    );
...
const Navbar = () => {
    ...
    return (
        <nav className="navbar-fixed-container">
            ...
            >
                {/* Header: Logo + Close */}
                {(() => { let fadeIndex = 0; return (<>
                <Fade in={isMobileMenuOpen} style={{ transitionDelay: isMobileMenuOpen ? `${fadeIndex++ * 50}ms` : '0ms' }}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                        <Link to={userData ? "/home" : "/"} style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '12px' }} onClick={() => setIsMobileMenuOpen(false)}>
                            <Box sx={{ width: 36, height: 36, bgcolor: 'primary.main', color: 'secondary.main', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '10px', fontWeight: 900, fontSize: '18px', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}>
                                V
                            </Box>
                            <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                                <Typography sx={{ fontSize: '20px', fontWeight: 900, letterSpacing: '-0.02em', color: 'primary.main', lineHeight: 1, my: 0 }}>
                                    INTERVU
                                </Typography>
                                <Typography sx={{ fontSize: '10px', fontWeight: 700, color: 'text.secondary', letterSpacing: '0.1em', my: 0 }}>
                                    PLATFORM
                                </Typography>
                            </Box>
                        </Link>
                ...
                {userData && (
                    <Fade in={isMobileMenuOpen} style={{ transitionDelay: isMobileMenuOpen ? `${fadeIndex++ * 50}ms` : '0ms' }}>
                        <Box sx={{ bgcolor: 'action.disabledBackground', borderRadius: '14px', p: 1.5, mb: 2, display: 'flex', alignItems: 'center', gap: 1.5 }}>
                            <Avatar src={userData.profilePicture} alt={userData.fullName} sx={{ width: 40, height: 40 }}>
                                {userData.fullName?.charAt(0)}
                            </Avatar>
                            <Box>
                                ...
                                <Typography sx={{ fontSize: '10px', fontWeight: 700, letterSpacing: '0.08em', color: 'text.disabled', textTransform: 'uppercase' }}>
                    ...
                    {currentMenuItems.map((item, idx) => {
                        if (item.type === "link") {
                            ...
                            if (item.sectionLabel) {
                                ...
                                elements.push(
                                    <Fade key={`sec-label-link-${idx}`} in={isMobileMenuOpen} style={{ transitionDelay: isMobileMenuOpen ? `${labelDelay * 50}ms` : '0ms' }}>
                                        <Typography sx={{ fontSize: '11px', fontWeight: 800, color: 'text.disabled', textTransform: 'uppercase', letterSpacing: '0.08em', px: 0.5, pt: 1.5, pb: 0.5 }}>
                                            {item.sectionLabel}
                                        </Typography>
                                    </Fade>
                                );
                            ...
                            elements.push(
                                <Fade key={`link-${idx}`} in={isMobileMenuOpen} style={{ transitionDelay: isMobileMenuOpen ? `${itemDelay * 50}ms` : '0ms' }}>
                                    <div>
                                        <MobileMenuItem item={item} userData={userData} onClose={() => setIsMobileMenuOpen(false)} />
                                    </div>
                                </Fade>
                            );
                        ...
                        if (item.sections) {
                            item.sections.forEach((section, secIdx) => {
                                const labelDelay = fadeIndex++;
                                elements.push(
                                    <Fade key={`sec-label-${idx}-${secIdx}`} in={isMobileMenuOpen} style={{ transitionDelay: isMobileMenuOpen ? `${labelDelay * 50}ms` : '0ms' }}>
                                        <Typography sx={{ fontSize: '11px', fontWeight: 800, color: 'text.disabled', textTransform: 'uppercase', letterSpacing: '0.08em', px: 0.5, pt: 1.5, pb: 0.5 }}>
                                            {section.title}
                                        </Typography>
                                    </Fade>
                                ...
                                section.items.forEach((sub, sIdx) => {
                                    const itemDelay = fadeIndex++;
                                    elements.push(
                                        <Fade key={`sec-item-${idx}-${secIdx}-${sIdx}`} in={isMobileMenuOpen} style={{ transitionDelay: isMobileMenuOpen ? `${itemDelay * 50}ms` : '0ms' }}>
                                            <div>
                                                <MobileMenuItem item={sub} userData={userData} onClose={() => setIsMobileMenuOpen(false)} />
                                            </div>
                                        </Fade>
                                    );
                                });
                            ...
                            const labelDelay = fadeIndex++;
                            elements.push(
                                <Fade key={`label-${idx}`} in={isMobileMenuOpen} style={{ transitionDelay: isMobileMenuOpen ? `${labelDelay * 50}ms` : '0ms' }}>
                                    <Typography sx={{ fontSize: '11px', fontWeight: 800, color: 'text.disabled', textTransform: 'uppercase', letterSpacing: '0.08em', px: 0.5, pt: 1.5, pb: 0.5 }}>
                                        {item.label}
                                    </Typography>
                                </Fade>
                            ...
                            item.items.forEach((sub, sIdx) => {
                                    const itemDelay = fadeIndex++;
                                    elements.push(
                                        <Fade key={`item-${idx}-${sIdx}`} in={isMobileMenuOpen} style={{ transitionDelay: isMobileMenuOpen ? `${itemDelay * 50}ms` : '0ms' }}>
                                            <div>
                                                <MobileMenuItem item={sub} userData={userData} onClose={() => setIsMobileMenuOpen(false)} />
                                            </div>
                                        </Fade>
                        ...
                        <Fade in={isMobileMenuOpen} style={{ transitionDelay: isMobileMenuOpen ? `${fadeIndex++ * 50}ms` : '0ms' }}>
                            <Box
                                ...
                                sx={{
                                    ...
                                    p: '10px 12px', textDecoration: 'none', borderRadius: '12px',
                                    color: 'text.secondary', fontSize: '14px', fontWeight: 600,
                                    transition: 'all 0.2s ease',
                        ...
                        <Fade in={isMobileMenuOpen} style={{ transitionDelay: isMobileMenuOpen ? `${fadeIndex++ * 50}ms` : '0ms' }}>
                            <Box
                                ...
                                sx={{
                                    ...
                                    p: '10px 12px', border: 'none', background: 'transparent', borderRadius: '12px',
                                    color: 'error.main', fontSize: '14px', fontWeight: 600, cursor: 'pointer',
                                    transition: 'all 0.2s ease',
