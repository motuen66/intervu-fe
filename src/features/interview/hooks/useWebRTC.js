...
const T0 = Date.now();
const tLog = (tag, ...args) => console.log(`[${tag} T+${Date.now() - T0}ms]`, ...args);
...
export function useWebRTC({ signalingSender, selfId }) {
    ...
    useEffect(() => {
        signalingSenderRef.current = signalingSender;
    }, [signalingSender]);
    ...
    const createPeerConnection = useCallback((targetId) => {
        ...
        pc.onicecandidate = ({ candidate }) => {
            if (candidate) {
                signalingSenderRef.current?.("SendIceCandidate", targetPeerIdRef.current, JSON.stringify(candidate));
            }
        };
        ...
        pc.onnegotiationneeded = async () => {
            ...
            try {
                ...
                signalingSenderRef.current?.("SendOffer", targetPeerIdRef.current, pc.localDescription.sdp);
    ...
    const handleOffer = useCallback(
        async (fromId, sdp) => {
            tLog(
                ...
                fromId,
                "| signalingState:",
                pcRef.current?.signalingState ?? "no-pc",
            );
    ...
    const handleAnswer = useCallback(
        async (fromId, sdp) => {
            ...
            try {
                ...
                if (!ignoreOffer.current) console.error("[WebRTC] setRemoteDescription(answer) error:", err);
            } finally {
                isSettingRemoteAnswerPending.current = false;
            }
            await flushIceCandidateQueue();
